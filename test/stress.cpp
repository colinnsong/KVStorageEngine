...
SkipList<string, string> skipList(18); // 创建一个最大层级为18的跳表实例
...
void *insertElement(void* threadid) {
    ...
    for (int i=tid*tmp, count=0; count<tmp; i++) {
        count++;
        skipList.insertNode(to_string(rand() % TEST_COUNT), "a"); // 随机生成一个键，并插入带有"a"的元素
    }
...
void *getElement(void* threadid) {
    ...
    for (int i=tid*tmp, count=0; count<tmp; i++) {
        count++;
        skipList.searchNode(to_string(rand() % TEST_COUNT)); // 随机生成一个键，并尝试检索
    }
    pthread_exit(NULL); // 退出线程
}
